...
export class AuthOutboxCronService implements OnModuleInit {
  private readonly logger = new Logger(AuthOutboxCronService.name);
  ...
  onModuleInit() {
    ...
    if (this.producer) {
      ...
      this.logger.log(`[DEBUG] setProducer called with ${this.producer.constructor.name}`);
    } else {
      this.logger.warn('[DEBUG] producer is null — outbox relay will not publish');
    }
  }
