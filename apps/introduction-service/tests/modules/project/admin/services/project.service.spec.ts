...
import { AdminProjectService } from '../../../../../src/modules/project/admin/services/project.service';
...
describe('AdminProjectService', () => {
  ...
  const mockItem = { id: 1n, name: 'Project 1', slug: 'project-1', status: 'active', images: [] };
  ...
  beforeEach(() => {
    ...
    service = new AdminProjectService(projectRepo as any, redis as any);
  });
  ...
  describe('getList', () => {
    it('should return paginated list', async () => {
      ...
      expect(projectRepo.findMany).toHaveBeenCalled();
  ...
  describe('getOne', () => {
    it('should return item when found', async () => {
      ...
      expect(projectRepo.findById).toHaveBeenCalledWith(1n);
    });
  ...
  describe('create', () => {
    it('should create with slug and clear cache', async () => {
      ...
      (SlugHelper.uniqueSlug as jest.Mock).mockResolvedValue('new-project');
    ...
    it('should throw BadRequestException on duplicate slug (P2002)', async () => {
      ...
      projectRepo.create.mockRejectedValue(p2002);
    ...
    it('should re-throw non-P2002 errors', async () => {
      projectRepo.create.mockRejectedValue(new Error('DB down'));
  ...
  describe('update', () => {
    ...
    it('should throw BadRequestException on duplicate slug (P2002)', async () => {
      const { Prisma } = require('src/generated/prisma');
      const p2002 = new Prisma.PrismaClientKnownRequestError('dup', { code: 'P2002' });
      projectRepo.update.mockRejectedValue(p2002);
      (SlugHelper.uniqueSlug as jest.Mock).mockResolvedValue('dup-slug');
  ...
  describe('delete', () => {
    it('should delete and clear cache', async () => {
      ...
      expect(projectRepo.delete).toHaveBeenCalledWith(1n);
      expect(redis.del).toHaveBeenCalled();
    });
