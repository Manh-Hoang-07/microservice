  ...
  async execute(code: string, options: ExecuteTemplateOptions) {
    ...
    if (!template) {
      throw new NotFoundException(
        `Content template with code ${code} not found`,
      );
    }

    if (template.status !== BasicStatus.active) {
      throw new BadRequestException(
        `Content template with code ${code} is not active`,
      );
    }

    if (template.category !== TemplateCategory.render) {
      throw new BadRequestException(
        `Template ${code} is of category ${template.category}, cannot be executed as render-type`,
      );
    }
    ...
    switch (template.type) {
      ...
      default:
        throw new BadRequestException(
          `Execution for template type ${template.type} is not yet implemented`,
        );
    }
  }
